...
При этом чтобы ограничить количество строк в выборке используется предложение LIMIT, которое указывается в конце запроса
и после него указывается количество строк, которое должно попасть в результаты выборки.

Для представленной ниже схемы airplanes напишите запрос, который вернет только те строки, в которых
в столбце model не содержится подстрока Airbus и Boeing. Ограничить результаты выборки необходимо одной строкой.
*/
