...
Необходимо вывести суммарный бюджет и количество фильмов для каждой кинокомпании, у которых хотя бы один фильм старше
2012 года. Должны быть выведены поля: название компании, общее число фильмов, суммарный бюджет.
...
SELECT fc.name AS name, COUNT(m.id) AS count, SUM(m.budget) AS sum
FROM movies m
