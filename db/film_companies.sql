...
Необходимо вывести среднюю продолжительность фильмов для каждого режиссера, у которых есть более 3 фильмов.
Должно быть выведено имя режиссера и средняя продолжительность фильмов. Группировка будет по имени режиссера.
*/
...
SELECT d.name, AVG(m.duration) as avg
FROM movies m
