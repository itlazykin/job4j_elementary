...
Если несколько значений в столбце совпадают, то для того, чтобы посчитать сумму среди уникальных значений,
то можно просто добавить DISTINCT. Выполнить запрос можно следующим образом:

SELECT SUM(DISTINCT имя_столбца) FROM название_таблицы;

Функция SUM игнорирует NULL. Т.е. эта функция не учитывает NULL при расчетах.

Для представленной ниже схемы transactions выполните запрос, который вернет общую сумму уникальных значений в столбце
amount.
*/
