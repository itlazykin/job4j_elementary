...
Необходимо подчеркнуть, что если значение в столбце для проверки меньше значения_1 или больше значения_2,
то логическое выражение вернет true и такие строки попадут в выборку. Иначе будет false.

Соответственно запрос без BETWEEN можно переписать следующим образом:

SELECT имена_столбцов FROM название_таблицы WHERE столбец_для_проверки < значение_1 OR столбец_для_проверки > значение_2

Для представленной ниже схемы, напишите запрос, который вернет только те строки,
в которых значение в столбце amount не находится в пределах от 100 до 200. Используйте NOT BETWEEN.
*/
